...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask() {
        ...
        int tasksSumTime = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(t -> Math.abs(Period.between(LocalDate.now(), t.getCreated()).getDays()))
                .reduce(0, (sum, d) -> sum + d);


        double avgTimePerTask = tasksSumTime / tasksQuantity;

        //Then
        Assert.assertEquals(3, tasksQuantity);
        Assert.assertEquals(10.0, avgTimePerTask,1);
    }
